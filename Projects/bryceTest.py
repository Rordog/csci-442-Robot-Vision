...
bbox = cv2.selectROI(trackArray, False) #(240,240,280,280)
...
try:
    ...
    while True:
        ...
        if ok:
            ...
            print("zdepth" + str(zDepth))
            if abs(zDepth - FOLLOW_DISTANCE) > FOLLOW_DISTANCE_THRESHOLD: # We need to move
                if zDepth < FOLLOW_DISTANCE:
                    # Move Backwards
                    throttle.set(-.5)
                else:
                    # Move forward
            ...
            else: # We are in the correct range
                throttle.reset()
                turn.reset()
